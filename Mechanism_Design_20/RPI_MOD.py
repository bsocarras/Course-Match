...
def RPI_MOD_recurse(vals, selected_indices, cap, n_knot):
    ...
    if n_hat < n_knot:
        ...
        for i in range(P_selected.shape[0]):
            first_part = (1.0-float(total_alloc[i]))
            for j in range(P_selected.shape[1]):
                second_part = cap[j]/(n_hat)
                P_selected[i][j] = float(P_selected[i][j]) + first_part*second_part

        # Recursively calling RPI_MOD_recurse
        ...
        return np.add(RPI_MOD_recurse(vals, selected_indices, cap, n_knot),  P)
...
def RPI_MOD(v, n_knot):
    ...
    return RPI_MOD_recurse(v, [], np.ones(v.shape[1]), n_knot)


# In[1]:


# changed 1/2 to nothing
